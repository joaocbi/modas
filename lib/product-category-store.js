...
import { getAllProducts, isAdminMutationEnabled, renameProductsCategory, renameProductsSubcategory } from "./product-store";
...
async function buildMergedProductCategories(categories) {
    return mergeCategoryCollections(categories);
}
...
function assertCategoryNameIsAvailable(categories, categoryName, ignoredCategoryId) {
    const duplicatedCategory = categories.find(
        (category) => getNormalizedKey(category.name) === getNormalizedKey(categoryName) && Number(category.id) !== Number(ignoredCategoryId)
    );
...
function assertSubcategoryNameIsAvailable(category, subcategoryName, ignoredSubcategoryId) {
    const duplicatedSubcategory = (category?.subcategories || []).find(
        (subcategory) =>
            getNormalizedKey(subcategory.name) === getNormalizedKey(subcategoryName) && Number(subcategory.id) !== Number(ignoredSubcategoryId)
    );
...
export async function updateProductCategory(id, input) {
    ...
    const currentCategory = categories.find((category) => Number(category.id) === normalizedId || getNormalizedKey(category.name) === getNormalizedKey(categoryName));

    if (!currentCategory) {
        throw new Error("Category not found.");
    }

    assertCategoryNameIsAvailable(categories, categoryName, currentCategory.id);
    ...
    if (hasDatabaseUrl()) {
        try {
            ...
            await renameProductsCategory(currentCategory.name, categoryName);
    ...
    const categoryIndex = fileCategories.findIndex((category) => Number(category.id) === Number(currentCategory.id));
    if (categoryIndex < 0) {
        throw new Error("Category not found.");
    }

    const normalizedCategory = normalizeCategory(fileCategories[categoryIndex], Number(fileCategories[categoryIndex].id) || normalizedId);
    const nextCategories = [...fileCategories];
...
export async function updateProductSubcategory(id, input) {
    if (!isAdminMutationEnabled()) {
        throw new Error("DATABASE_URL is required for admin writes in production.");
    }
    ...
    assertSubcategoryName(subcategoryName);
    const categories = await getAllProductCategories();
    const category = categories.find((item) => Number(item.id) === Number(input?.categoryId));
    const currentCategory = category || categories.find((item) => item.subcategories.some((subcategory) => Number(subcategory.id) === normalizedId));
    const currentSubcategory = currentCategory?.subcategories.find((subcategory) => Number(subcategory.id) === normalizedId);

    if (!currentCategory || !currentSubcategory) {
        throw new Error("Subcategory not found.");
    }

    assertSubcategoryNameIsAvailable(currentCategory, subcategoryName, currentSubcategory.id);
    ...
    if (hasDatabaseUrl()) {
        try {
            ...
            await renameProductsSubcategory(currentCategory.name, currentSubcategory.name, subcategoryName);
    ...
    const fileCategories = await readProductCategoriesFromFile();
    const categoryIndex = fileCategories.findIndex((item) => Number(item.id) === Number(currentCategory.id));

    if (categoryIndex < 0) {
        throw new Error("Category not found.");
    }

    const normalizedCategory = normalizeCategory(fileCategories[categoryIndex], Number(fileCategories[categoryIndex].id) || Number(currentCategory.id));
    const nextCategories = [...fileCategories];
    nextCategories[categoryIndex] = {
        ...normalizedCategory,
        subcategories: sortByName(
            normalizedCategory.subcategories.map((subcategory) =>
                Number(subcategory.id) === normalizedId
                    ? {
                          ...subcategory,
                          name: subcategoryName,
                      }
