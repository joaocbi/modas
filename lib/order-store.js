...
export async function getOrderById(id) {
    ...
    if (hasDatabaseUrl()) {
        ...
        console.log("[OrderStore] Loaded order from database.", { id: normalizedId });
        return mapDatabaseOrder(order);
    }

    const orders = await getAllOrders();
    const order = orders.find((currentOrder) => Number(currentOrder.id) === normalizedId) || null;
    console.log("[OrderStore] Loaded order from file fallback.", { id: normalizedId, found: Boolean(order) });
    return order;
}
...
export async function updateOrder(id, input) {
    ...
    if (hasDatabaseUrl()) {
        const normalizedId = Number(id);
        const existingOrder = await getOrderById(normalizedId);

        if (!existingOrder) {
            throw new Error("Order not found.");
        }

        const order = normalizeOrder({ ...existingOrder, ...input }, normalizedId);
