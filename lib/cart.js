...
function normalizeCartItem(item) {
    ...
    const selectedStrapShoulder = String(item?.selectedStrapShoulder || "").trim();
    const selectedFabricPattern = String(item?.selectedFabricPattern || "").trim();
...
export function addCartItem(item) {
    ...
    const existingItemIndex = currentItems.findIndex(
        (currentItem) =>
            ...
            currentItem.selectedStrapShoulder === nextItem.selectedStrapShoulder &&
            currentItem.selectedFabricPattern === nextItem.selectedFabricPattern
    );
...
export function buildCartItemKey(item) {
    return [
        ...
        item.selectedSize,
        item.selectedColor,
        item.selectedStrapShoulder,
        item.selectedFabricPattern,
    ].join("::");
}
